...
本协议供 ZCode 中的 AI 执行。用户可以只提供仓库 URL，本地无需预先存在仓库。目标版本固定为 `v4.0.0`，仓库固定为 `https://github.com/tony-apan/zcode_skills`。

> [!NOTE]
> release audit 和 PR-only 规则只约束仓库维护者的 push 和发版。普通用户安装、更新或卸载 `v4.0.0` 不需要运行 `release_gate.py`、安装 Git hook、创建 PR 或调用 `github` 智能体，不影响现有安装流程。
...
写入前识别当前 OS，并确认 Git、Python >= 3.9 可执行；Windows 另确认 PowerShell >= 5.1。请求 tag 必须是固定的 `v4.0.0`，不是分支或其他版本。无法验证的前提应停止并报告。
...
- `version` 已是 `4.0.0` 且用户未明确要求重装：停止写入并报告已安装。
- 版本不同：进入更新流程。
...
```sh
WORKTREE=$(mktemp -d "${TMPDIR:-/tmp}/tony-agents-pack-v4.0.0.XXXXXX")
git clone --branch v4.0.0 --single-branch --depth 1 https://github.com/tony-apan/zcode_skills "$WORKTREE"
...
```powershell
$Worktree = Join-Path $env:TEMP ("tony-agents-pack-v4.0.0-" + [guid]::NewGuid().ToString("N"))
git clone --branch v4.0.0 --single-branch --depth 1 https://github.com/tony-apan/zcode_skills $Worktree
git -C $Worktree describe --tags --exact-match HEAD
git -C $Worktree status --short
```

核验输出必须精确包含 `v4.0.0`，且新 clone 应为干净工作树。clone 后切换到仓库根目录并重新读取 `INSTALL-FOR-AI.md`；继续执行时仍受用户发送的主提示词约束。
...
只能通过 `scripts/model_inventory.py` 生成脱敏 inventory；macOS/Linux 的固定路径示例为 `/tmp/tony-agents-model-inventory.json`，实际执行仍必须使用本次 `mktemp` 生成的唯一临时目录。
...
更新必须使用阶段 1 已核验的固定 tag `v4.0.0` 仓库。默认保留当前有效 model/thoughtLevel；只有用户明确要求“重新分配模型”时，才执行阶段 2 并在 update 中传 `--model-map`。
...
重新分配时，在两条 update 命令后增加 `--model-map` 及阶段 2 实际生成的路径。v4.0.0 将 `shencha-content` 的调用与报告主字段从单值 `review_profile` 升级为集合 `review_profiles`，并新增可组合 `editorial` 与必填 `review_tier`。旧字段可临时映射为单元素集合，但应迁移到新字段；迁移检查 `review_profiles` 为非空去重集合、仅含合法 token，专业文章、case study 与 research report 必须包含 `editorial`。`sheyun` 的两处交审调用同步迁移为 `review_profiles=[social]`；这个 breaking 只影响调用/报告字段，普通安装 state schema 不变，本次 changed agents 为 `sheyun` 与 `shencha-content`，其他 18 个 agent 不变。安装器会更新两者正文，同时默认保留当前有效的本地 `model`/`thoughtLevel`；管理器继续三方合并并保护其他本地修改。update dry-run 出现冲突或本地修改时逐条报告；三方合并冲突必须人工处理，不得自动取任一侧。冲突或 Git merge 不可用时保留原文件并生成 `*.tony-agents-pack.incoming`，不得自动覆盖。若 update 报告 `Reinstalled missing`，说明该文件之前被删除，现已按包内版本恢复。正式更新前会创建覆盖新旧集合的 snapshot。
...
- 20 个 agent 各自绑定的模型，以及“上下文未知”等所有能力降级及原因
...
- 安装后逐个抽查 20 个 agent 的 frontmatter 可解析；`github` 的 tools 不含 Bash/Write/Edit 且 `disallowedTools` 含三者；`frontend` 无 `skills` 字段；更新时确认 `shencha-content` 正文已升级且原有本地 `model`/`thoughtLevel` 保留
- 提醒用户新建 ZCode 会话生效
...
用户直接提出更新时，仍先执行阶段 0 并检查默认 state 路径，再按需执行阶段 1。state 同为 `4.0.0` 时不重复；旧版本按阶段 3 更新。除非用户明确要求重新分配，否则禁止运行 inventory，且 update 不传 model-map。必须报告 incoming、本地修改保护和临时 clone 清理结果。

## 卸载流程

卸载先检查默认 state `~/.zcode/agents/.tony-agents-pack/state.json`（Windows 为 `%USERPROFILE%\.zcode\agents\.tony-agents-pack\state.json`），再执行阶段 1 获取并核验 `v4.0.0`。确认 state 的 `package` 是 `tony-agents-pack` 后，先 dry-run 并解释计划，再正式执行。
