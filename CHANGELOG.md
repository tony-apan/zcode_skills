...
- package fingerprint 的 tracked 文件类型与 executable marker 改从 Git index/tree mode 读取，并覆盖 Git 检测到的当前 mode change，避免 Windows `chmod` 语义与 POSIX 不一致；PASS 审计前要求先暂存所有非 ignored 发布文件。
